...
// handleSchedulerPick selects a qoder auth candidate based on the
// panel-selected active account. Non-qoder candidates are always deferred
...
func handleSchedulerPick(raw []byte) ([]byte, error) {
	...
	// Collect qoder candidates only.
	var qdCandidates []pluginapi.SchedulerAuthCandidate
	for _, c := range req.Candidates {
		...
		qdCandidates = append(qdCandidates, c)
	}
	if len(qdCandidates) == 0 {
		return okEnvelope(pluginapi.SchedulerPickResponse{Handled: false})
	}

	// Build thin view for active-auth picker.
	cands := make([]activeAuthCandidate, 0, len(qdCandidates))
	for _, c := range qdCandidates {
